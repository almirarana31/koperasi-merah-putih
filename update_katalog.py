...
export default function KatalogKementerianPage() {
  ...
  const chartRows = catalogRows.map((row) => ({
    ...
    volume: Math.round((row.stockKg * scaleFactor) / 100),
  }))
  ...
  return (
    <div className="space-y-5">
      <div className="space-y-2">
        ...
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900">Katalog Agregat Komoditas</h1>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          { label: 'Pemasok Terhubung', value: supplierCount.toLocaleString('id-ID'), sub: 'Koperasi Pemasok Aktif', icon: Users, tone: 'blue' },
          { label: 'Cakupan Wilayah', value: regionCoverage.toLocaleString('id-ID'), sub: 'Wilayah Sumber Komoditas', icon: Sparkles, tone: 'slate' },
        ].map((stat, i) => (
      ...
      <div className="space-y-5">
        <Card className="rounded-none border-slate-200 shadow-sm overflow-hidden border-t-4 border-t-slate-900">
          <CardHeader className="bg-slate-50/50 border-b border-slate-100">
            ...
            <CardDescription className="text-[10px] font-bold text-slate-500 uppercase">Ketersediaan Komoditas Lintas Pemasok (Unit: 100 KG)</CardDescription>
          </CardHeader>
          <CardContent className="h-[320px] p-4 pt-6">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartRows}>
                ...
                />
                <Bar dataKey="volume" name="Volume" fill="#ca8a04" radius={[0, 0, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
