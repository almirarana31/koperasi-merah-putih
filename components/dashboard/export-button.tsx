  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="sm" className="h-9 border-slate-300 text-xs font-medium text-slate-700 shadow-[0_8px_18px_-16px_rgba(15,23,42,0.2)] hover:text-slate-900" disabled={isExporting}>
      ...
      <DropdownMenuContent align="end" className="w-40">
        <DropdownMenuItem onClick={handlePDF} className="cursor-pointer text-xs font-medium text-slate-700">
          <FileText className="h-4 w-4 mr-2 text-rose-600" />
          Format PDF
        </DropdownMenuItem>
        <DropdownMenuItem onClick={handleExcel} className="cursor-pointer text-xs font-medium text-slate-700">
