        ...
        <div className="flex flex-wrap gap-2">
           <div className="flex rounded-xl border border-[var(--dashboard-secondary)]/12 bg-white p-1 shadow-sm">
            <Button 
              ...
              className={`h-8 rounded-lg px-4 text-xs font-semibold  transition-all ${activeView === 'war-room' ? 'bg-[var(--dashboard-primary)] text-white shadow-sm' : 'text-[var(--dashboard-secondary)] hover:bg-[var(--dashboard-neutral)]'}`}
            >
              War Room
            </Button>
            <Button 
              ...
              className={`h-8 rounded-lg px-4 text-xs font-semibold  transition-all ${activeView === 'monitoring' ? 'bg-[var(--dashboard-primary)] text-white shadow-sm' : 'text-[var(--dashboard-secondary)] hover:bg-[var(--dashboard-neutral)]'}`}
            >
              Monitoring
            </Button>
            <Button 
              ...
              className={`h-8 rounded-lg px-4 text-xs font-semibold  transition-all ${activeView === 'audit' ? 'bg-[var(--dashboard-primary)] text-white shadow-sm' : 'text-[var(--dashboard-secondary)] hover:bg-[var(--dashboard-neutral)]'}`}
            >
              Audit Log
            </Button>
          </div>
          <Button onClick={handleExportPDF} variant="outline" size="sm" className="h-10 border-[var(--dashboard-secondary)]/18 text-xs font-semibold   text-[var(--dashboard-secondary)]" disabled={isExporting}>
            <FileText className="mr-2 h-4 w-4 text-[var(--dashboard-primary)]" />
            Export Audit PDF
          </Button>
          <Button size="sm" className="h-10 bg-[var(--dashboard-secondary)] px-6 text-xs font-semibold   text-white shadow-[0_10px_24px_-12px_rgba(69,90,100,0.45)] hover:bg-[#394B54]">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-6">
              ...
              <div className="flex flex-col">
                <span className="text-xs font-semibold text-white/65  ">Coverage</span>
                <span className="text-xs font-semibold text-white">{totals.activeUnits.toLocaleString()} Active Units</span>
              </div>
              <div className="flex flex-col">
                <span className="text-xs font-semibold text-white/65  ">Population</span>
                <span className="text-xs font-semibold text-white">{totals.totalMembers.toLocaleString()} Members Sync</span>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <Badge variant="outline" className="h-6 border-white/15 bg-white/10 text-xs font-semibold   text-[var(--dashboard-tertiary)]">99.9% Uptime</Badge>
              <span className="text-xs font-semibold text-white/70  ">Last Update: {new Date().toLocaleTimeString("id-ID")}</span>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <div className="space-y-6">
            ...
            <Card className="border-none shadow-[0_4px_12px_-4px_rgba(0,0,0,0.05)] overflow-hidden">
              <CardHeader className="p-4 border-b border-slate-50 bg-slate-50/50">
                <div className="flex items-center justify-between">
                  ...
                  <Badge className="border-none bg-[var(--dashboard-tertiary)]/20 text-xs font-semibold  text-[#8A5F00]">+12.4% VOLUME</Badge>
                </div>
              </CardHeader>
            ...
            <Card className="border-none shadow-[0_4px_12px_-4px_rgba(0,0,0,0.05)] overflow-hidden">
              <CardHeader className="p-4 border-b border-slate-50 bg-slate-50/50">
                <div className="flex items-center justify-between">
                  ...
                  <Badge className="border-none bg-[var(--dashboard-secondary)] text-xs font-semibold   text-white">LIVE FLOW</Badge>
                </div>
              </CardHeader>
                ...
                {[
                  ...
                  <div key={i} className="p-4 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-between">
                    <div>
                      ...
                      <p className={`mt-1 text-lg font-semibold  ${item.tone === 'rose' ? 'text-[var(--dashboard-primary)]' : item.tone === 'secondary' ? 'text-[var(--dashboard-secondary)]' : 'text-[#A57500]'}`}>
                        Rp {(item.val / 1000000).toFixed(1)} JT
                      </p>
                    </div>
                    <Badge variant="outline" className={`text-xs font-semibold ${item.tone === 'rose' ? 'border-[var(--dashboard-primary)]/15 text-[var(--dashboard-primary)]' : item.tone === 'secondary' ? 'border-[var(--dashboard-secondary)]/15 text-[var(--dashboard-secondary)]' : 'border-[var(--dashboard-tertiary)]/30 text-[#8A5F00]'}`}>
                      {item.trend}
                    </Badge>
                  </div>
                ))}
          ...
          <Card className="border-none shadow-[0_4px_12px_-4px_rgba(0,0,0,0.05)] overflow-hidden">
             <CardHeader className="border-b border-[#394B54] bg-[var(--dashboard-secondary)] p-4">
               <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-xs font-semibold text-white  ">Distribusi & Logistik Nasional</CardTitle>
                    <CardDescription className="text-xs font-bold text-white/70">Monitoring pengiriman lintas provinsi</CardDescription>
                  </div>
                  <Button size="sm" variant="outline" className="h-8 border-white/15 bg-white/10 text-xs font-semibold  text-white hover:bg-white/15">Detail Rute</Button>
               </div>
             </CardHeader>
                ...
                {[
                  ...
                  <div key={i} className="p-4 rounded-xl border border-slate-100 bg-white hover:border-slate-300 transition-all group">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <p className={`text-xs font-semibold  mt-2 ${reg.status === 'On-Time' ? 'text-[var(--dashboard-secondary)]' : 'text-[var(--dashboard-primary)]'}`}>{reg.status}</p>
                    </div>
                  </div>
                ))}
             </CardContent>
        ...
        <div className="space-y-6">
          <Card className="flex h-full max-h-[800px] flex-col overflow-hidden border-none bg-[var(--dashboard-secondary)] text-white shadow-[0_20px_40px_-16px_rgba(69,90,100,0.6)]">
            <CardHeader className="border-b border-white/10 bg-[#394B54] p-5">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-xs font-semibold text-[var(--dashboard-tertiary)] ">STREAMING</span>
                </div>
              </div>
            </CardHeader>
                ...
                {logs.map((log, i) => (
                  <div key={i} className="group cursor-pointer p-5 transition-colors hover:bg-[#394B54]">
                    <div className="flex items-center justify-between mb-2">
                      <Badge className={`text-xs font-semibold  h-4 px-1.5 rounded border-none ${
                      ...
                      <span className="text-xs font-mono text-white/45">{log.time}</span>
                    </div>
                    <p className="text-sm font-semibold text-white/90  leading-tight transition-colors group-hover:text-[var(--dashboard-tertiary)]">{log.action}</p>
                    <div className="flex items-center justify-between mt-2">
                      <p className="text-xs font-bold text-white/55 ">USR: {log.role}</p>
                      <p className="text-xs font-semibold text-white/45  ">{log.entity}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="border-t border-white/10 bg-[#394B54] p-4">
              <Button variant="ghost" className="h-10 w-full text-xs font-semibold text-white/70   hover:text-white">
              ...
              {[
                ...
                <div key={i} className="p-3 bg-white rounded-lg border border-rose-100 flex items-center justify-between shadow-sm">
                  <div>
                    ...
                    <p className="text-xs font-bold text-slate-400   mt-0.5">{risk.scope}</p>
                  </div>
                  <Badge variant="outline" className={`text-xs font-semibold  ${risk.impact === 'High' ? 'text-rose-600 border-rose-100' : 'text-slate-500 border-slate-100'}`}>
                    {risk.impact}
                  </Badge>
                </div>
              ))}
