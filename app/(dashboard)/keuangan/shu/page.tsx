    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-2">
          <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tight">NATIONAL SHU AUDIT</h1>
          <p className="text-[10px] font-black text-slate-500 mt-1 uppercase tracking-widest leading-relaxed">
            SURPLUS MONITORING & DISTRIBUTION COMPLIANCE • BOOK YEAR 2023 • {formatCurrency(stats.totalMargin)} AGGREGATE SURPLUS
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" size="sm" className="h-9 text-[10px] font-black uppercase tracking-widest border-slate-200 text-slate-600 rounded-none" onClick={() => toast({ title: "Audit Sync", description: "Verifying surplus allocation against national AD/ART standards..." })}>
            <History className="h-3.5 w-3.5 mr-2 text-blue-600" />
            HISTORY
          </Button>
          <Button size="sm" className="h-9 bg-slate-900 text-white hover:bg-slate-800 text-[10px] font-black uppercase tracking-widest px-6 rounded-none shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] transition-all" onClick={() => toast({ title: "Export Initiation", description: "Generating cross-entity distribution manifest..." })}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'TOTAL SURPLUS (SHU)', value: (stats.totalMargin / 1000000000).toFixed(2), sub: 'BILLION IDR', icon: TrendingUp, tone: 'emerald' },
          { label: 'PORSI TERDISTRIBUSI', value: (stats.distributedSHU / 1000000000).toFixed(2), sub: 'BILLION IDR', icon: Wallet, tone: 'blue' },
          { label: 'BASIS ANGGOTA', value: stats.activeMembers.toLocaleString(), sub: 'RECIPIENTS', icon: Users, tone: 'slate' },
          { label: 'EFFICIENCY RATIO', value: '82.4%', sub: 'NATIONAL AVG', icon: Activity, tone: 'emerald' },
        ].map((s, i) => (
        ...
        <div className="space-y-6">
           <Card className="border-none shadow-sm overflow-hidden rounded-none">
              ...
              <CardHeader className="p-6 border-b border-slate-50 bg-slate-50/50">
                 <div className="flex items-center justify-between">
                    <div>
                       <CardTitle className="text-sm font-black uppercase tracking-widest text-slate-900">National Allocation Matrix</CardTitle>
                       <CardDescription className="text-[10px] font-bold text-slate-400 uppercase mt-1">Aggregate distribution based on collective AD/ART</CardDescription>
                    </div>
                    <Badge className="bg-emerald-100 text-emerald-700 text-[9px] font-black border-none px-2 h-5 uppercase rounded-none tracking-widest">LIVE AUDIT</Badge>
                 </div>
              </CardHeader>
              <CardContent className="p-6">
                 <div className="grid gap-8 md:grid-cols-2">
                    ...
                    <div className="bg-slate-900 rounded-none p-6 text-white flex flex-col justify-center relative overflow-hidden shadow-2xl">
                       ...
                       <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Gross Revenue Pool</p>
                       <p className="text-3xl font-black mt-2 tracking-tighter">{formatCurrency(stats.totalRevenue)}</p>
                       <div className="mt-8 space-y-4">
                          <div className="flex items-center justify-between border-b border-white/10 pb-2">
                             <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Operating Costs</span>
                             <span className="text-sm font-black text-rose-400">-{formatCurrency(stats.totalRevenue * 0.8)}</span>
                          </div>
                          <div className="flex items-center justify-between">
                             <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Net Surplus (SHU)</span>
           ...
           <Card className="border-none bg-white shadow-sm overflow-hidden rounded-none">
              ...
              <CardHeader className="p-6 border-b border-slate-50">
                 <CardTitle className="text-sm font-black uppercase tracking-widest text-slate-900">Member Distribution Log</CardTitle>
                 <CardDescription className="text-[10px] font-bold text-slate-400 uppercase mt-1">Individual surplus allocation per active participant</CardDescription>
              </CardHeader>
              <CardContent className="p-0">
                 <Table>
                    <TableHeader className="bg-slate-900">
                       <TableRow className="hover:bg-slate-900 border-none">
                          <TableHead className="h-10 text-[10px] font-black uppercase tracking-widest text-slate-400 px-6">RECIPIENT</TableHead>
                          <TableHead className="h-10 text-[10px] font-black uppercase tracking-widest text-slate-400 px-6">REGION</TableHead>
                    ...
                    <TableBody>
                       {filteredSHU.map((item) => (
                          <TableRow key={item.nama} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors group">
                             ...
                             <TableCell className="px-6 py-4 text-right">
                                <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-300 group-hover:text-slate-900 rounded-none group-hover:bg-white group-hover:shadow-sm" onClick={() => toast({ title: "Allocation Audit", description: "Loading detailed surplus calculation for " + item.nama })}>
        ...
        <div className="space-y-6">
              ...
              <CardHeader className="p-4 border-b border-white/5 bg-slate-900/50">
                 <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                       ...
                       <span className="text-[9px] font-black text-emerald-500 tracking-widest">LIVE AUDIT</span>
                    </div>
                 </div>
              </CardHeader>
              <CardContent className="p-0">
                 <div className="divide-y divide-white/5">
                    {[
                      ...
                      <div key={i} className="p-4 hover:bg-white/5 transition-colors cursor-pointer group">
                         ...
                         <p className="text-[9px] font-bold text-slate-500 mt-1 uppercase">REGION: {log.region}</p>
                      </div>
                    ))}
                 </div>
                 <div className="p-4 bg-white/5 border-t border-white/5">
                    <Button variant="ghost" className="w-full text-[10px] font-black text-slate-500 hover:text-white uppercase tracking-widest h-9 rounded-none" onClick={() => toast({ title: "Master Console", description: "Loading national financial audit console..." })}>
                       FINANCIAL CONSOLE →
                    </Button>
                 </div>
              </CardContent>
           ...
           <Card className="border-none shadow-sm bg-slate-50 rounded-none">
              <CardHeader className="p-4 border-b border-slate-200">
                 <CardTitle className="text-[10px] font-black uppercase tracking-widest text-slate-900">FINANCIAL PERFORMANCE</CardTitle>
              </CardHeader>
              <CardContent className="p-4 space-y-4">
                 {[
                   { label: 'PROFITABILITY INDEX', val: '1.24', trend: 'up' },
                   { label: 'CAPITAL RESERVE', val: 'Rp 42.8B', trend: 'stable' },
      ...
      <Card className="border-none overflow-hidden relative group cursor-pointer bg-rose-50 border border-rose-100 rounded-none" onClick={() => toast({ title: "Investigation Triggered", description: "Opening forensic audit for operational margin variance in Bali region..." })}>
          ...
          <div className="flex items-center gap-6">
             ...
             <div>
                <div className="flex items-center gap-3">
                   <Badge className="bg-rose-600 text-white text-[10px] font-black px-2 h-5 rounded-none tracking-widest border-none">AUDIT ALERT</Badge>
                   <span className="text-[10px] font-black text-rose-900 uppercase tracking-widest">Margin Variance Detected (&gt;15%)</span>
                </div>
                <p className="mt-2 text-lg font-black text-rose-900 uppercase tracking-tight">CRITICAL: Operational margin deviation in Bali regional hub. Immediate audit required.</p>
             </div>
          </div>
