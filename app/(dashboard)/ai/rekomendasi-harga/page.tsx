    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs font-bold  text-slate-500  mt-2">
              REKOMENDASI HARGA STRATEGIS BERDASARKAN INDEKS SUPPLY-DEMAND REGIONAL
            </p>
          </div>
          <div className="flex items-center gap-2">
            <Button variant="outline" className="font-semibold border-2 text-[10px] uppercase tracking-widest h-8 rounded-none border-slate-200 px-3">
              <History className="mr-1.5 h-3.5 w-3.5" /> PRICE HISTORY
            </Button>
            <Button className="bg-slate-900 font-semibold text-[10px] uppercase tracking-widest h-8 rounded-none px-3">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        {processedData.recommendations.map((item, idx) => (
          <Card 
            key={item.komoditas} 
            className={`border-none transition-all cursor-pointer hover:bg-slate-50 group overflow-hidden rounded-none ${selectedIdx === idx ? 'ring-2 ring-slate-900 bg-white shadow-md' : 'bg-white shadow-sm'}`}
            onClick={() => setSelectedIdx(idx)}
          >
            <div className={`h-1 w-full ${item.action === 'Naik' ? 'bg-emerald-500' : 'bg-rose-500'}`} />
            <CardHeader className="pb-2 p-4">
              <CardDescription className="text-[10px] font-black uppercase tracking-widest text-slate-500">{item.komoditas}</CardDescription>
            ...
            <CardContent className="p-4 pt-0">
              <div className="flex items-center justify-between">
                <Badge className={`text-[10px] font-black h-4 px-1.5 border-0 rounded-none ${item.action === 'Naik' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
                ...
                <span className="text-[10px] font-black text-slate-300 uppercase">vs Rp {Math.floor(item.hargaSaat).toLocaleString()}</span>
              </div>
            </CardContent>
          </Card>
        ))}
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2 border-none shadow-sm rounded-none">
          <CardHeader className="border-b border-slate-100 bg-slate-50/50">
            <CardTitle className="text-xs font-black uppercase tracking-widest text-slate-900">Tren Harga Pasar</CardTitle>
            <CardDescription className="text-[10px] font-bold text-slate-500 uppercase">PERGERAKAN HARGA AGREGAT MINGGUAN LINTAS WILAYAH</CardDescription>
          </CardHeader>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={processedData.trends}>
                ...
                <Tooltip 
                  ...
                  itemStyle={{ fontSize: "9px", fontWeight: "600", textTransform: "uppercase" }}
                  formatter={(value: number) => `Rp ${Math.floor(value).toLocaleString()}`}
                />
                <Legend iconType="rect" wrapperStyle={{ paddingTop: "20px", fontSize: "9px", fontWeight: "600" }} />
        ...
        <Card className="border-none shadow-sm flex flex-col bg-white rounded-none">
          <CardHeader className="border-b border-slate-100 bg-slate-50/50">
            <CardTitle className="text-xs font-black uppercase tracking-widest text-slate-900">Optimization Matrix</CardTitle>
            <CardDescription className="text-[10px] font-bold text-slate-500 uppercase">SELECTED: {selectedItem.komoditas.toUpperCase()}</CardDescription>
          </CardHeader>
          <CardContent className="p-6 flex-1 flex flex-col justify-between">
            <div className="space-y-8">
              <div className="space-y-3">
                ...
                <div className="h-4 w-full bg-slate-50 rounded-none border border-slate-100 relative overflow-hidden">
              ...
              <div className="space-y-2">
                <div className="p-3 bg-slate-50/50 border border-slate-100 rounded-none flex items-center justify-between group-hover:bg-white transition-colors">
                ...
                <div className="p-3 bg-slate-900 text-white rounded-none flex items-center justify-between shadow-lg">
            ...
            <div className="mt-8 space-y-4 pt-6 border-t border-slate-50">
              <div className="flex items-center gap-3">
                <div className="h-9 w-9 bg-emerald-50 rounded-none flex items-center justify-center border border-emerald-100 shrink-0">
                  <DollarSign className="h-4 w-4 text-emerald-600" />
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">IMPACT ANALYSIS</p>
                  <p className="text-[10px] font-black text-slate-700 leading-tight uppercase">PROJECTED +RP {Math.floor(selectedItem.rekomendasi - selectedItem.hargaSaat).toLocaleString()}/UNIT IMPROVEMENT</p>
                </div>
              </div>
              <Button className="w-full font-black text-[10px] uppercase tracking-widest bg-slate-900 h-10 rounded-none hover:bg-slate-800 transition-all">
                IMPLEMENT STRATEGY
              </Button>
            </div>
          </CardContent>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card className="border-none shadow-sm overflow-hidden bg-white rounded-none">
          <CardHeader className="border-b border-slate-100 bg-slate-50/50 p-4">
            <CardTitle className="text-xs font-black uppercase tracking-widest text-slate-900 leading-none">Ekuilibrium Supply-Demand</CardTitle>
          </CardHeader>
          <CardContent className="pt-6">
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={processedData.demandSupply}>
                ...
                />
                <Legend iconType="rect" wrapperStyle={{ paddingTop: "20px", fontSize: "9px", fontWeight: "600" }} />
                <Bar dataKey="demand" fill="#10b981" name="DEMAND" radius={[0, 0, 0, 0]} barSize={24} />
                <Bar dataKey="supply" fill="#0f172a" name="SUPPLY" radius={[0, 0, 0, 0]} barSize={24} />
                <Bar dataKey="optimal" fill="#f59e0b" name="OPTIMAL" radius={[0, 0, 0, 0]} barSize={24} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>

        <Card className="border-none bg-slate-900 text-white p-6 flex flex-col justify-between shadow-xl relative overflow-hidden rounded-none">
          <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
            <BrainCircuit className="h-32 w-32 text-emerald-400" />
          </div>
          <div className="space-y-6 relative z-10">
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <h3 className="text-sm font-black uppercase tracking-tight flex items-center gap-2 leading-none">
                <BrainCircuit className="h-5 w-5 text-emerald-400" />
                AI STRATEGY INSIGHT
              </h3>
              <Badge className="bg-emerald-500 text-slate-900 font-black text-[10px] uppercase tracking-widest px-2 h-5 border-0 rounded-none">HIGH IMPACT</Badge>
            </div>
            <div className="space-y-4">
              <div className="space-y-1.5">
                <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest">PRIMARY STRATEGY</p>
                <p className="text-[10px] font-bold text-slate-300 leading-relaxed uppercase tracking-wider">
                  <span className="text-white font-black underline decoration-emerald-400/50 underline-offset-2">BERAS GRADE A</span> SHOWS HIGH PRICE INELASTICITY. RECOMMEND +8% PRICE ADJUSTMENT TO CAPTURE CONSUMER SURPLUS.
                </p>
              </div>
              <div className="space-y-1.5">
                <p className="text-[10px] font-black text-amber-400 uppercase tracking-widest">DEFENSIVE PLAY</p>
                <p className="text-[10px] font-bold text-slate-300 leading-relaxed uppercase tracking-wider">
                  <span className="text-white font-black underline decoration-amber-400/50 underline-offset-2">CABAI MERAH</span> SUPPLY IS PEAKING. LOWER PRICE BY 6% PREEMPTIVELY TO PREVENT INVENTORY STALE-MATE.
                </p>
              </div>
            </div>
          </div>
          <div className="mt-6 pt-4 border-t border-slate-800 flex items-center justify-between relative z-10">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-emerald-400">AGGREGATE MARGIN LIFT: +14.2%</span>
            </div>
